...
def predict_image(image):
    ...
    nutrition = nutritional_values.get(predicted_label, {})
    scientific_name = scientific_names.get(predicted_label, "Unknown")
    description = Info.get(predicted_label, "No description available.")
    category = "Fruit" if predicted_label in fruits else ("Vegetable" if predicted_label in vegetables else "Medicinal Plant")
...
st.write("Upload an image to classify an ingredient.")
...
if uploaded_file:
    ...
    # Display results
    st.write(f"### Predicted Ingredient: {result['prediction']}")
    st.write(f"**Scientific Name:** {result['scientific_name']}")
    st.write(f"**Category:** {result['category']}")

    st.write("### Nutritional Values:")
    for key, value in result['nutritional_values'].items():
        st.write(f"- **{key}:** {value}")

    st.write(f"### General Info:\n{result['description']}")